...
4. 确认两个正式分支的 `质量门禁` 已通过；1.20.1 必须通过 Java 17、21、25，1.21.1 必须通过 Java 21、25 的服务端运行矩阵。
...
- 两个正式分支最近一次持续集成中的 `质量门禁` 均成功，且发布工作流的十组正式 Java 运行矩阵全部通过。
