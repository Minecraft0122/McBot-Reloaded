...
- 为 Fabric 与 NeoForge 增加 Java 21、25 真实服务端启动、McBot 加载和正常关服测试。
- Gradle 继续使用 JDK 21，服务端运行进程使用独立 JDK，避免把构建工具兼容性误判为模组运行兼容性。
...
- 修复没有展示信息的玩家进度触发空 `Optional` 并崩服（上游 #187、#190、#196）。
- 移植 1.20.1 的群名片、`@`、换行、删群、空回复、绑定、权限、数据和停服稳定性修复。
- 支持 `ws://`、`wss://` 与无协议 OneBot 地址，并拒绝错误协议（上游 PR #197）。
- 有界等待并隔离异步命令响应；复杂命令输出使用纯文本并按行分段。
- 作者列表增加 Minecraft0122，统一仓库联系信息和中文文档。
- 删除未启用的 Forge 模块、重复 CI、失效发布流程和旧本地机器人残留。
